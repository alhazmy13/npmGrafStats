...
tail -F /logs/proxy-host-*_access-geo.log | grep --line-buffered -E "([0-9]{1,3}[\.]){3}[0-9]{1,3}" | while read line;
...
do
  ...
  then
    ...
    python /root/.config/NPMGRAF/Getipinfo.py "$outsideip" "$targetdomain" "$length" "$targetip" "ReverseProxyGeoIPsConnections" "$measurementtime" "$allowedip" "$asndb"
  fi
done
reboot
